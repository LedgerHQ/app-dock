...
parser_error_t _readOptionStatementKind_V2(parser_context_t* c, pd_OptionStatementKind_V2_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readStatementKind_V2(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionTupleBalanceOfBalanceOfBlockNumber_V2(parser_context_t* c, pd_OptionTupleBalanceOfBalanceOfBlockNumber_V2_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readTupleBalanceOfBalanceOfBlockNumber_V2(c, &v->contained))
    }
    return parser_ok;
}
    ...
    uint8_t* pageCount)
{
    ...
    outValue[offset++] = ' ';
    CHECK_PARSER_ERR(_toStringConviction_V2(&v->conviction,
        outValue + offset,
        outValueLen - offset,
        pageIdx,
        pageCount));
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringStatementKind_V2(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringTupleBalanceOfBalanceOfBlockNumber_V2(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
